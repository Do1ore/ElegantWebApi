...
namespace ElegantWebApi.Application.Features.AddDataList
{
    public class AddDataListCommandValidator : AbstractValidator<AddDataListCommand>
    {
        ...
        public override Task<ValidationResult> ValidateAsync(ValidationContext<AddDataListCommand> context,
            CancellationToken cancellation = default)
        {
            var result = base.ValidateAsync(context, cancellation);
            if (context.InstanceToValidate.ListModel.Values != null)
            {
                var myDateTime = context.InstanceToValidate.ListModel.ExpirationTime;
                var maxExpirationTime =
                    Convert.ToInt32(_configuration.GetSection("DefaultExpirationTime")["MaxEpirationTimeInMinutes"]);
                var defaultExpirationTime =
                    Convert.ToInt32(
                        _configuration.GetSection("DefaultExpirationTime")["DefaultExpirationTimeInMinutes"]);

                if (myDateTime == default | myDateTime < DateTime.Now |
                    GetMinutesDifference(myDateTime) > maxExpirationTime)
                {
                    context.InstanceToValidate.ListModel.ExpirationTime =
        ...
        private int GetMinutesDifference(DateTime givenDateTime)
        {
